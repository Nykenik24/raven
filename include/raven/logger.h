...
#define RVN_INFO(...)  do { if (raven_debug_enabled) raven_log(LOG_INFO, __FILE__, __LINE__, __VA_ARGS__); } while(0)

#define RVN_DEBUG_LOG(...) do { if (raven_debug_enabled) raven_log(LOG_DEBUG, __FILE__, __LINE__, __VA_ARGS__); } while(0)
#define RVN_TRACE_LOG(...) do { if (raven_debug_enabled) raven_log(LOG_TRACE, __FILE__, __LINE__, __VA_ARGS__); } while(0)
