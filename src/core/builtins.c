...
void builtin_gen_call(void* ctx_ptr, const char* name, void* args_ptr, int arg_count) {
    ...
    switch (info->type) {
        case BUILTIN_SYSCALL: {
            ...
            if (arg_count >= 2) inst_pop_reg(ctx->emit, REG_EBX);  
            if (arg_count >= 3) inst_pop_reg(ctx->emit, REG_ECX);  
            if (arg_count >= 4) inst_pop_reg(ctx->emit, REG_EDX);  
            if (arg_count >= 5) inst_pop_reg(ctx->emit, REG_ESI);  
            if (arg_count >= 6) inst_pop_reg(ctx->emit, REG_EDI);  
