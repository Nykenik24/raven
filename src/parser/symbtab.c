...
static unsigned int sym_hash(const char* name, size_t len) __attribute__((unused));
...
struct rvn_symbol* sym_declare(sym_table* table, symbol_kind kind, const char* name, size_t len, rvn_node* decl) {
...
struct rvn_symbol* sym_lookup_current(sym_table* table, const char* name, size_t len) {
    if (!table || !table->current) return NULL;
...
struct rvn_symbol* sym_lookup_recursive(sym_scope* scope, const char* name, size_t len) {
    while (scope) {
        struct rvn_symbol* sym = scope->symbols;
        while (sym) {
            if (sym->name_len == len && memcmp(sym->name, name, len) == 0) {
