...
rvn_type* type_i8(void) {
    return type_primitive(TYPE_I8, 1, 1);
}

rvn_type* type_i16(void) {
    return type_primitive(TYPE_I16, 2, 2);
}

rvn_type* type_i32(void) {
    return type_primitive(TYPE_I32, 4, 4);
}

rvn_type* type_i64(void) {
    return type_primitive(TYPE_I64, 8, 8);
}

rvn_type* type_u8(void) {
    return type_primitive(TYPE_U8, 1, 1);
}

rvn_type* type_u16(void) {
    return type_primitive(TYPE_U16, 2, 2);
}

rvn_type* type_u32(void) {
    return type_primitive(TYPE_U32, 4, 4);
}

rvn_type* type_u64(void) {
    return type_primitive(TYPE_U64, 8, 8);
}
...
rvn_type* type_char(void) {
    return type_primitive(TYPE_CHAR, 1, 1);
}
...
rvn_type* type_slice(rvn_type* elem) {
    rvn_type* type = type_primitive(TYPE_SLICE, sizeof(void*) * 2, sizeof(void*));
    if (!type) return NULL;
...
rvn_type* type_function(type_list* params, rvn_type* ret, bool is_variadic) {
    rvn_type* type = type_primitive(TYPE_FUNCTION, sizeof(void*), sizeof(void*));
    if (!type) return NULL;
...
void type_list_add(type_list* list, rvn_type* type) {
    ...
    if (list->count >= list->capacity) {
        ...
        rvn_type** new_items = realloc(list->items, new_capacity * sizeof(rvn_type*));
        if (!new_items) return;
...
void type_field_list_add(type_field_list* list, const char* name, size_t len, rvn_type* type) {
    if (!list) return;
