...
static int ensure_bool_capacity(rvn_flag_ctx* ctx) {
    if (ctx->bool_count >= ctx->bool_capacity) {
        ...
        rvn_flag_bool* new_array = realloc(ctx->bool_flags, sizeof(rvn_flag_bool) * new_capacity);
        if (!new_array) {
...
static int ensure_string_capacity(rvn_flag_ctx* ctx) {
    if (ctx->string_count >= ctx->string_capacity) {
        ...
        rvn_flag_string* new_array = realloc(ctx->string_flags, sizeof(rvn_flag_string) * new_capacity);
        if (!new_array) {
...
void flags_register_bool(rvn_flag_ctx* ctx, const char* name, char short_name, const char* description, bool* value) {
    if (!ctx) return;
    if (ensure_bool_capacity(ctx) < 0) return;
...
void flags_register_string(rvn_flag_ctx* ctx, const char* name, char short_name, const char* description, char** value) {
    if (!ctx) return;
    if (ensure_string_capacity(ctx) < 0) return;
